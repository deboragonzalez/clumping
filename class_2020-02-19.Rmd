...
- When you keep sampling variables, they will tend towards the mean, butvariability stays roughly the same because of an error parameter. (??)

## The Null

People in Gov1006 sit randomly.

--> Picking new variables is dangerous because we can pick any given observable characteristic. Can result in stastistically significant patterns in the noise. 

## Treatment Effect of Women:
...
sim_clumps <- sim %>% 
  group_by(clumps) %>% 
  count() %>% 
  mutate(clumps_prop = (n/10000)*100)
```
...
sim %>% 
  ggplot() + 
    geom_bar(mapping = aes(x = clumps, y = ..prop..), 
             width = 0.1) +
    labs(title = "Women Clumping in Gov 1006",
         subtitle = "Roughly 25% of simulations have 1 single gender tables",
...
## Grade Adjustment on Midterms

```{r midterm complaints}

# Grade adjustment on midterms and grade improvements in students' grades

# -Treatment: giving the break = adjust grades
    # Counterfactual = not adjusting grades --> can't see the causal effect of this
    # Factual = adjusting grades --> can see the causal effect

    # Effect = difference between performance when given or not given treatment

# Outcome: better future grades? 

# Causal inference? 

# Is this really causal or is this selection bias?


n <- 1000

midterm_grade <- rnorm(n, 50,10)
